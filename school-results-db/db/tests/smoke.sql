...
WITH u AS (UPDATE exams.marks m SET remarks = 'x' FROM exams.exam_components ec
      ...
      WHERE m.component_id = ec.component_id AND sb.code = 'ENG' RETURNING 1)
SELECT 'RLS: teacher(Maths) updating ENGLISH rows touches:' AS check, COUNT(*) AS rows_touched FROM u;
